...
On coalescent simulations of a hierarchical demography, three-way direction accuracy rises from the $33\%$ chance level at the smallest migration rates to $96.6\%$ at the largest simulated rate ($2.5\times10^{-4}$) and $99.7\%$ across the appreciable band above it, with a rate-averaged accuracy of $75.6\%$. A detect-then-orient gate separates appreciable migration from the no-migration control at ROC-AUC $0.99$ (all-rate $0.80$) with a well-calibrated score (expected calibration error $0.03$). The discriminative signal lies in the features rather than the architecture: a linear (logistic) classifier on the same statistics is at least as accurate as the deep network---and no deeper, kernel, or random-matrix method improves on it---as our theory predicts, since the linear head is Bayes-optimal to first order. A precomputed Patterson's $D$ cannot orient gene flow. In a new four-population benchmark containing every one of the 12 ordered donor--recipient edges, training-fold-only population-relabeling symmetrization raises exact edge accuracy from $50.0\%$ to $62.9\%$ (chance $8.3\%$); raw-kernel, neural, and random-matrix-whitened neural kernels do not improve on the linear rule. Independent same-prior simulations reproduce the strong-rate result, but nuisance and natural-panel controls delimit transfer. In \textit{Heliconius}, Patterson's $D$ and the raw pair-private sharing ratio distinguish four selected geographic trios from an intended allopatric control, whereas the frozen direction head calls the same class on all five panels, including the control with an uncalibrated score of $0.992$. High confidence on these off-distribution panels is therefore not external calibration. Migration-rate \emph{magnitude}, by contrast, is the regression half of the problem and the place where the more expressive network does beat a linear model---mean relative error $27.8\%$ versus $93.1\%$ in the appreciable band (Spearman $0.75$ versus $0.66$)---because the rate-to-feature map is genuinely nonlinear; magnitude remains recoverable in rank but not in absolute scale below a Fisher threshold. Direction is thus a linear-classification problem and magnitude a nonlinear-regression one (Table~\ref{tab:lin-vs-net}). A supporting theory establishes that the rarefaction statistics are linear functionals of the joint site-frequency spectrum, that they orient gene flow where symmetric outgroup-free summaries provably cannot, and that a linear classifier is first-order optimal; its many-direction extension shows that the standardized separation cost of $K$ ordered edges grows only as $\sqrt{\log K}$ when the minimum Fisher separation is retained.
\end{abstract}
...
\begin{tabular}{llcc}
\toprule
Task & Metric & Linear & Network \\
\midrule
Direction (classification) & accuracy (\%) & \textbf{79.0} & 73.8 \\
Magnitude (regression) & MRE (\%), appreciable & 93.1 & \textbf{27.8} \\
Magnitude (regression) & Spearman, appreciable & 0.66 & \textbf{0.75} \\
\bottomrule
\end{tabular}
\caption{Linear model versus network on the two halves of the problem, under identical leakage-free
$\mathrm{GroupKFold}(5)$ replicate splits and preprocessing. A linear head is at least as accurate as the
network on direction---the first-order-linear classification, where our theory predicts no classifier
beats linear---while the network substantially outperforms the linear model on magnitude, the nonlinear
regression (lower MRE is better).}
...
itself with the kernel holds it at $79.0\%$. The standard remedies for an underperforming network do not recover the linear head on direction either: a residual network with an explicit linear skip warm-started from the logistic solution, a linear initialization, and heavy regularization all sit at $75$--$76\%$, short of the linear head. A linear skip guarantees the linear solution only if the nonlinear path is held at zero, but at this information limit that path has only noise to fit, and training spends its capacity on that noise at a cost to test accuracy. The robust choice for direction is therefore the linear head itself. On magnitude the network matches the median-regression
