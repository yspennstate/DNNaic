...
\begin{abstract}
Introgression---the transfer of genetic material between diverged populations---is a major force shaping diversity and adaptation, yet inferring its \emph{direction} remains hard. Classical statistics such as Patterson's $D$ and the $f$-statistics need an outgroup, lose power for ancient or subtle gene flow, and cannot by construction say which population donated to which. We introduce DNNaic, a two-stage fully connected neural network built on nine allelic rarefaction statistics---allelic richness, private allelic richness, and pairwise-private allelic richness across three populations, each summarized across loci by its mean, variance, and standard error---computed with PADZE, the open-source rarefaction tool we release with this work. The first stage detects appreciable gene flow and the second orients it, from the same outgroup-free feature vector. Because rarefaction yields $198$ correlated feature rows per simulated replicate, we evaluate only under a replicate-aware protocol in which no replicate is shared between training and test, a split we prove is the mathematically correct one.

On coalescent simulations of a hierarchical demography, three-way direction accuracy rises from the $33\%$ chance level at the smallest migration rates to $96.6\%$ at the largest simulated rate ($2.5\times10^{-4}$) and $99.7\%$ across the appreciable band above it, with a rate-averaged accuracy of $75.6\%$. A detect-then-orient gate separates appreciable migration from the no-migration control at ROC-AUC $0.99$ (all-rate $0.80$) with a well-calibrated score (expected calibration error $0.03$), so a direction is asserted only where it is reliable. The discriminative signal lies in the features rather than the architecture---a logistic model on the same statistics matches the network, as our theory predicts---while a precomputed Patterson's $D$, which by construction cannot orient gene flow, labels direction below the majority-class rate. The result is stable across seeds, reproduces on independently simulated replicate batches the model never saw, and transfers to held-out demographies; applied to the \textit{Heliconius} butterfly complex, where the direction of adaptive introgression is documented, it recovers the known donor-to-recipient flow ($\textit{melpomene}\!\to\!\textit{timareta}$) that a symmetric Patterson's $D$ detects but cannot orient. Migration-rate \emph{magnitude}, by contrast, is recoverable in rank but not in scale below a rate threshold that we characterize with a Fisher-information bound, so the validated deliverable is the direction of appreciable gene flow. A supporting theory establishes that the rarefaction statistics are linear functionals of the joint site-frequency spectrum, that they orient gene flow where symmetric outgroup-free summaries provably cannot, and that a linear classifier on them is first-order optimal---recasting the pipeline as a principled, interpretable method that complements both classical summary statistics and heavier genotype-matrix networks.
\end{abstract}
...
validated deliverable of this work is the direction of appreciable gene flow.
...
Continuous migration was introduced to explore three introgression scenarios among the experimental populations (P1, P2, P3), together with a no-migration control: Case A, migration from P1 into P2 (backwards in time); Case B, from P2 into P3; Case C, from P3 into P2; and Case D, no introgression. Cases B and C are the same population pair with the flow reversed, so telling them apart is the donor-versus-recipient orientation that a symmetric statistic such as Patterson's $D$ cannot make.
...
Second, to fill in the spectrum between the fixed rates, we drew $300$ additional rates from an exponential distribution with mean $2.5\times10^{-4}$, assigned each draw to a uniformly chosen non-control direction, and simulated five replicates at that rate. This yields $1{,}500$ continuously-sampled replicates ($500$ per direction) spanning a biologically plausible range of low per-generation rates and lets the network learn behaviour between the fixed grid points.
...
In our simulation study, we sampled 200 individuals at the end of each replicate from each of the three populations. Thus, running a PADZE analysis on one replicate results in 198 individual allelic richness vectors, one for each standardized sample size $g \in [2, 199]$:
...
variance, and standard-error outputs and additionally exposes the across-loci skewness
and excess kurtosis used as features here. The package reads VCF directly (as well as
...
zero-migration control D---with per-replicate probability aggregation and six independent random
...
$99.7\%$ (six-seed mean, standard deviation $0.1\%$); averaged over all positive rates it is
...
\begin{figure}[H]
  ...
  (leakage-controlled, six-seed mean) climbs monotonically with the per-generation migration rate,
...
boosted/network column is the six-seed figure of Section~\ref{subsec:benchmarks}; the linear column
...
orient it only where detection succeeds---the method is both accurate and well calibrated. The
detection gate reaches ROC-AUC $0.99$ in the appreciable regime ($0.80$ across all rates), with
expected calibration error $0.03$ and Brier score $0.11$, so its confidence scores can be read
directly to set an operating point. The direction head supports the same selective use: ranking
...
is stable across all six seeds (standard deviations at or below $0.3\%$), and an independent
...
Fisher-information limit made precise in Theorem~\ref{thm:fisher}; the validated deliverable of this
work is the direction of appreciable gene flow.
...
itself with the kernel holds it at $79.0\%$. On magnitude the network matches the median-regression
baseline in the recoverable band (appreciable mean relative error $28\%$, Spearman $0.75$ with the
true rate) and the kernel correction does not improve it. A positive control rules out an inert
...
cannot call these detections right or wrong. The decisive test is instead a labelled empirical system
with an independently published gene-flow direction; because \textsc{PADZE} runs directly on sequence
data, that test is a matter of data, not tooling.

\subsection{Real-data validation: directional introgression in \textit{Heliconius}}
\label{subsec:heliconius}

We then ran that test. The \textit{Heliconius melpomene}--\textit{timareta}--\textit{cydno} complex
is a textbook case of adaptive introgression in which \textit{H.~melpomene} is the documented donor
of red wing-pattern alleles to the co-mimics \textit{H.~timareta} and \textit{H.~cydno}
\citep{pardodiaz2012adaptive,wallbank2016evolutionary,martin2013speciation}, so the direction of
gene flow between \textit{melpomene} and \textit{timareta} is established independently of any model.
From whole-genome sequence data for the three species together with \textit{H.~numata} as an
outgroup \citep{martin2013speciation}, we drew $25{,}000$ genome-wide biallelic SNPs and mapped the
trio to the caterpillar tree as $P_1=\textit{cydno}$, $P_2=\textit{timareta}$,
$P_3=\textit{melpomene}$---the same \textsc{PADZE} ingestion that builds the model features also
feeds the classical baseline, so the two read identical sites.

Patterson's $D$ registers the gene flow overwhelmingly---$D(\textit{cydno},\textit{timareta},
\textit{melpomene};\textit{numata})=+0.51$ ($Z=+33$, block jackknife over chromosomes)---but, being
symmetric under exchange of donor and recipient, it cannot say which way the flow runs: the
reciprocal ordering of the sister pair returns the same magnitude with the opposite sign. DNNaic
orients it. The detection gate fires (mean appreciable probability $1.0$), and both the linear and
the nonlinear direction heads call class $C$ ($P_3\!\to\!P_2$, that is,
\textit{melpomene}$\to$\textit{timareta}) with probability $1.0$ and $0.998$---the documented
donor-to-recipient direction. The call is a property of the whole genome rather than of a few loci:
$C$ is the modal class in all ten disjoint genome blocks, and it is corroborated model-free by the
rarefaction summaries themselves, in which the pair-private allelic richness shared by
\textit{timareta} and \textit{melpomene} exceeds that shared by \textit{cydno} and \textit{melpomene}
by $2.6$-fold---the labelled asymmetry that a symmetric $D$ discards (Theorem~\ref{thm:symmetric}).
Confidence tracks the strength of the real gene flow: moving the less-introgressed \textit{cydno},
allopatric to the Amazonian \textit{melpomene} race sampled here, into the recipient position lowers
the orientation confidence rather than reversing it. The call is stable across race combinations:
for three further trios pairing \textit{cydno} and \textit{timareta} with different \textit{melpomene}
races the signal is strong ($D=0.40$--$0.48$, all $Z>15$) and both heads again return
\textit{melpomene}$\to$\textit{timareta} at probability $0.98$--$1.00$, while a trio that pairs
\textit{timareta} with an allopatric \textit{melpomene} race not co-occurring with it shows
essentially no gene flow ($D=0.01$), so the classical signal itself tracks real co-occurrence.
Because the network is trained only on the
coalescent simulations and applied here off its training distribution, we read this as a validation
on a labelled real system rather than a calibrated accuracy; the agreement of four independent
lines---$D$, the model-free feature asymmetry, both trained heads, and the published biology---makes
the orientation call robust.
...
The detection reproduces cleanly, with the population contrast it should have. Patterson's
$D(\text{YRI},\text{CEU},\text{Neanderthal};\text{ancestral})=+0.27$ ($Z=3.4$, block jackknife over
chromosomes; ABBA $=76$ against BABA $=44$): the European shares an excess of derived alleles with
...
exactly as it must---between two African populations
$D(\text{YRI},\text{LWK},\text{Neanderthal})=+0.03$ with ABBA $\approx$ BABA, and between two Eurasian
populations $D(\text{CEU},\text{CHB},\text{Neanderthal})=-0.06$, the near-zero and faintly
...
distribution, is unaffected and still detects. The learned rarefaction orientation is thus matched to
population-scale samples that resemble the training regime---the \textit{Heliconius} species panels of
Section~\ref{subsec:heliconius} and the injected-signal human trios of Section~\ref{subsec:real-data},
each with many individuals per population---while few-genome, deeply divergent archaic introgression is
detectable (by $D$) but not orientable from the frozen model's features, a scope boundary the gate
enforces without supervision.
...
\[
1-\!\!\sum_{\beta'\neq\beta}\!Q\!\big(\tfrac12 m\sqrt{L}\,\lVert W(\Delta_\beta-\Delta_{\beta'})\rVert\big)
\ \le\ A_\beta(m)\ \le\ 1-\tfrac12\max_{\beta'\neq\beta}Q\!\big(\tfrac12 m\sqrt{L}\,\lVert W(\Delta_\beta-\Delta_{\beta'})\rVert\big),
\]
so it is controlled by the \emph{smallest} pairwise separation, with orientation threshold
$m_\star^{\mathrm{dir}}=c\big/\!\big(\sqrt{L}\,\min_{\beta\neq\beta'}\lVert W(\Delta_\beta-\Delta_{\beta'})\rVert\big)$.
...
analysis in the number of independent loci. On a labelled real system---the \textit{Heliconius}
adaptive-introgression complex, where \textit{H.~melpomene} is the known donor of wing-pattern
alleles to \textit{H.~timareta}---the frozen model recovers that documented direction where a
symmetric Patterson's $D$ ($D=0.51$, $Z=33$) detects the gene flow but cannot orient it
(Section~\ref{subsec:heliconius}). None of these changes the qualitative conclusion that compact
rarefaction summaries carry a direction signal that classical symmetric statistics cannot access.

\section{Limitations and scope}
\label{sec:limitations}
We delimit the scope of the present study.
\begin{itemize}
\item \textbf{Direction, not magnitude.} The validated deliverable is the direction of appreciable
gene flow. Migration-rate \emph{magnitude} is information-limited at small rates: positive-rate
...
though it attenuates when coalescence is fast and divergence shallow. The frozen model recovers the
documented direction on a labelled real system---\textit{melpomene}$\to$\textit{timareta} in the
\textit{Heliconius} complex used to develop the ABBA--BABA test \citep{durand2011testing}
(Section~\ref{subsec:heliconius})---but that call is made off the training distribution, so a
model matched to the target taxon (or a broader panel of labelled systems, such as the
\textit{Anopheles gambiae} complex) would strengthen the external validation. The complementary
limit is visible on the Neanderthal system (Section~\ref{subsec:neanderthal}): there the frozen model
...
outside the training distribution. The rarefaction route to orientation is thus best suited to
population-scale, in-distribution samples, and the calibrated gate is what keeps the method from
over-reaching when they are not available.
